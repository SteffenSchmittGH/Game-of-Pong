...
#ifndef SC_PONG_SIGNAL_H_INCLUDED
#define SC_PONG_SIGNAL_H_INCLUDED
 ...
 *  I-Port
 */
typedef RC_t (*SC_PONG_driverIn_t)(SC_PONG_data_t * const);

/**
 *  Definition of function types for driver interfaces.
 *  O-Port
 */
typedef RC_t (*SC_PONG_driverOut_t)( const SC_PONG_data_t * const);
...
/**
 * Instances of the struct SC_PONG_cfg_t represent the signal object configuration.
 */
struct SC_PONG_cfg_s
{
    /**
     * Function pointer to the input driver
     */
    SC_PONG_driverIn_t const inDriver;

    /**
     * Function pointer to the output driver table
     */
   SC_PONG_driverOut_t const outDriver;
...
typedef const struct SC_PONG_cfg_s SC_PONG_cfg_t;
...
/**
 * Instances of the struct SC_PONG_t represent the signal objects.
...
{                                                                                                                      
     ...
     */
	const SC_PONG_cfg_t * const    pSigCfg;

};                                                                                                                     
typedef struct SC_PONG_s SC_PONG_t;
 ...
 *
 * @param  SC_PONG_t * const signal: Pointer to the signal object which will be initialised
 * @param const sc_pong_data_t * const value: Pointer to initial value of the application data
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_PONG_init( SC_PONG_t * const signal, const SC_PONG_data_t * const value )
{                                                                                                                      
 ...
 *
 * @param  SC_PONG_t * const signal: Pointer to the signal object
 * @return  sig_status_t: The current signal status
 *
 **/
static inline RTE_signalStatus_t RTE_SC_PONG_getStatus( const SC_PONG_t * const signal )
{
 ...
 *
 */
static inline RC_t RTE_SC_PONG_setStatus( SC_PONG_t * const signal, const RTE_signalStatus_t status )
{
 ...
 */
static inline uint32_t RTE_SC_PONG_getAge( const SC_PONG_t * const signal )
{
   return signal->age;
}
 ...
 * @return void
 */
static inline void RTE_SC_PONG_incAge(  SC_PONG_t * const signal , uint32_t increment)
{
	   //Hack....
 ...
 *
 * @param  SC_PONG_t * const signal: Pointer to the signal object
 * @return sc_pong_data_t: current value of the application data buffer
 *
 **/
static inline SC_PONG_data_t RTE_SC_PONG_get( const SC_PONG_t * const signal )
{               
 ...
 *
 * @param  SC_PONG_t * const signal: Pointer to the signal object
 * @param  SC_PONG_data_t* data: Pointer to the return data Pointer to the signal object
 * @return RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_PONG_getThreadSafe( const SC_PONG_t * const signal, SC_PONG_data_t * const data )
{    
 ...
 *
 * @param  SC_PONG_t * const signal: Pointer to the signal object
 * @param  sc_pong_data_t const value: value for the application data buffer
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_PONG_set(SC_PONG_t * const signal, const SC_PONG_data_t value  )
{
 ...
 *
 */
static inline RC_t RTE_SC_PONG_pullPort( SC_PONG_t * const signal )
{                                                                                                                      
   RC_t ret = RC_SUCCESS;
   ...
   {
	  SC_PONG_data_t value = signal->value;
      ret = signal->pSigCfg->inDriver(&value);
                                                                                                                       
      if ( ret == RC_SUCCESS )                                                                                         
      {
    	  //Critical Section
    	  RTE_SC_PONG_set(signal, value);
      }
      else
      {
		  //Signal is invalid. Error event (if present) will be fired
		  RTE_SC_PONG_setStatus(signal, RTE_SIGNALSTATUS_INVALID);
      }
 ...
 *
 **/
static inline RC_t RTE_SC_PONG_pushPort( const SC_PONG_t * const signal )
{                                                                                                                      
   RC_t ret = RC_ERROR;
