...
#ifndef SC_EVENT_SIGNAL_H_INCLUDED
#define SC_EVENT_SIGNAL_H_INCLUDED
 ...
 *  I-Port
 */
typedef RC_t (*SC_EVENT_driverIn_t)(SC_EVENT_data_t * const);

/**
 *  Definition of function types for driver interfaces.
 *  O-Port
 */
typedef RC_t (*SC_EVENT_driverOut_t)( const SC_EVENT_data_t * const);
...
/**
 * Instances of the struct SC_EVENT_cfg_t represent the signal object configuration.
 */
struct SC_EVENT_cfg_s
{
    /**
     * Function pointer to the input driver
     */
    SC_EVENT_driverIn_t const inDriver;

    /**
     * Function pointer to the output driver table
     */
   SC_EVENT_driverOut_t const outDriver;
...
typedef const struct SC_EVENT_cfg_s SC_EVENT_cfg_t;
...
/**
 * Instances of the struct SC_EVENT_t represent the signal objects.
...
{                                                                                                                      
     ...
     */
	const SC_EVENT_cfg_t * const    pSigCfg;

};                                                                                                                     
typedef struct SC_EVENT_s SC_EVENT_t;
 ...
 *
 * @param  SC_EVENT_t * const signal: Pointer to the signal object which will be initialised
 * @param const sc_event_data_t * const value: Pointer to initial value of the application data
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_EVENT_init( SC_EVENT_t * const signal, const SC_EVENT_data_t * const value )
{                                                                                                                      
 ...
 *
 * @param  SC_EVENT_t * const signal: Pointer to the signal object
 * @return  sig_status_t: The current signal status
 *
 **/
static inline RTE_signalStatus_t RTE_SC_EVENT_getStatus( const SC_EVENT_t * const signal )
{
 ...
 *
 */
static inline RC_t RTE_SC_EVENT_setStatus( SC_EVENT_t * const signal, const RTE_signalStatus_t status )
{
 ...
 */
static inline uint32_t RTE_SC_EVENT_getAge( const SC_EVENT_t * const signal )
{
   return signal->age;
}
 ...
 * @return void
 */
static inline void RTE_SC_EVENT_incAge(  SC_EVENT_t * const signal , uint32_t increment)
{
	   //Hack....
 ...
 *
 * @param  SC_EVENT_t * const signal: Pointer to the signal object
 * @return sc_event_data_t: current value of the application data buffer
 *
 **/
static inline SC_EVENT_data_t RTE_SC_EVENT_get( const SC_EVENT_t * const signal )
{               
 ...
 *
 * @param  SC_EVENT_t * const signal: Pointer to the signal object
 * @param  SC_EVENT_data_t* data: Pointer to the return data Pointer to the signal object
 * @return RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_EVENT_getThreadSafe( const SC_EVENT_t * const signal, SC_EVENT_data_t * const data )
{    
 ...
 *
 * @param  SC_EVENT_t * const signal: Pointer to the signal object
 * @param  sc_event_data_t const value: value for the application data buffer
 * @return RC_t: standard return code, currently only RC_SUCCESS
 *
 **/
static inline RC_t RTE_SC_EVENT_set(SC_EVENT_t * const signal, const SC_EVENT_data_t value  )
{
 ...
 *
 */
static inline RC_t RTE_SC_EVENT_pullPort( SC_EVENT_t * const signal )
{                                                                                                                      
   RC_t ret = RC_SUCCESS;
   ...
   {
	  SC_EVENT_data_t value = signal->value;
      ret = signal->pSigCfg->inDriver(&value);
                                                                                                                       
      if ( ret == RC_SUCCESS )                                                                                         
      {
    	  //Critical Section
    	  RTE_SC_EVENT_set(signal, value);
      }
      else
      {
		  //Signal is invalid. Error event (if present) will be fired
		  RTE_SC_EVENT_setStatus(signal, RTE_SIGNALSTATUS_INVALID);
      }
 ...
 *
 **/
static inline RC_t RTE_SC_EVENT_pushPort( const SC_EVENT_t * const signal )
{                                                                                                                      
   RC_t ret = RC_ERROR;
